...
- 公司级规范必须收敛在 `company-public/rules/` 中。
- 部门级通用资源必须收敛在各部门的公共 Skill 中（如 `sztech-frontend/common-skill/`）。
...
1. **全局规范遵循**：请结合用户环境中挂载的 `company-public/rules/` 来规范你的代码输出。
2. **工作流输出 (如 SDD)**：
   - **如果**用户处于 SDD (OpenSpec) 工作流中，请结合 `@skills/company-public/wf-sdd-mode` 规范你的输出。
...
2. **防腐化 (Anti-corruption)**：通过将业务黑话锁定在 `references/`，全局规范锁定在外部。Skill 变成了一个干净的纯逻辑处理节点。
